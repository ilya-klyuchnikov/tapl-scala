...
/** `PackratParsers` is a component that extends the parser combinators provided by
  ...
  *
  * Packrat Parsing is a technique for implementing backtracking, recursive-descent parsers, with
  * the advantage that it guarantees unlimited lookahead and a linear parse time. Using this
  * technique, left recursive grammars can also be accepted.
  *
  * Using `PackratParsers` is very similar to using `Parsers`:
  *   - any class/trait that extends `Parsers` (directly or through a subclass) can mix in
  *     `PackratParsers`. Example: `'''object''' MyGrammar '''extends''' StandardTokenParsers
  *     '''with''' PackratParsers`
  *   - each grammar production previously declared as a `def` without formal parameters becomes a
  *     `lazy val`, and its type is changed from `Parser[Elem]` to `PackratParser[Elem]`. So, for
  *     example, `'''def''' production: Parser[Int] = {...}` becomes `'''lazy val''' production:
  *     PackratParser[Int] = {...}`
  *   - Important: using `PackratParser`s is not an ''all or nothing'' decision. They can be free
  *     mixed with regular `Parser`s in a single grammar.
  *
  * Cached parse results are attached to the ''input'', not the grammar. Therefore,
  * `PackratsParser`s require a `PackratReader` as input, which adds memoization to an underlying
  * `Reader`. Programmers can create `PackratReader` objects either manually, as in
  * `production('''new''' PackratReader('''new''' lexical.Scanner("input")))`, but the common way
  * should be to rely on the combinator `phrase` to wrap a given input with a `PackratReader` if the
  * input is not one itself.
  *
  * @see
...
trait PackratParsers extends Parsers {
  ...
  /** A specialized `Reader` class that wraps an underlying `Reader` and provides memoization of
    * parse results.
    */
  class PackratReader[+T](underlying: Reader[T]) extends Reader[T] { outer =>
    ...
    private[PackratParsers] def updateCacheAndGet[T2](
        p: Parser[T2],
        w: MemoEntry[T2],
    ): MemoEntry[T2] = {
      cache.put((p, pos), w)
  ...
  /** A parser generator delimiting whole phrases (i.e. programs).
    *
    * Overridden to make sure any input passed to the argument parser is wrapped in a
    * `PackratReader`.
    */
  ...
  /** Implicitly convert a parser to a packrat parser. The conversion is triggered by giving the
...
to update each parser involved in the recursion.
  ...
  /** Explicitly convert a given parser to a memoizing packrat parser. In most cases, client code
    * should avoid calling `memo` directly and rely on implicit conversion instead.
    */
  ...
  private def grow[T](p: super.Parser[T], rest: PackratReader[Elem], head: Head): ParseResult[T] = {
    ...
    rest.recursionHeads.put(
      rest.pos,
      head, /*match {case Head(hp,involved,_) => Head(hp,involved,involved)}*/
    )
