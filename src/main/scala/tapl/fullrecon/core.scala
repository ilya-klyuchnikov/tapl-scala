...
object Typer {
  ...
  def recon(ctx: Context, nextUVar: UVarGenerator, t: Term): (Ty, UVarGenerator, Constr) =
    t match {
      case TmVar(i, _) =>
  ...
  def applySub(ctr: IdConstr, tyT: Ty): Ty = {
    ctr.reverse.foldLeft(tyT)((tyS, ctr) =>
      ctr match { case (TyId(tyX), tyC2) => substInTy(tyX, tyC2, tyS) }
    )
  }
